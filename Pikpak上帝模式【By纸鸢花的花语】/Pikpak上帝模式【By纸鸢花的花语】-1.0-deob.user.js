...
(function () {
    ...
    function check_sign() {
      ...
      let signedString = generateUppercaseLetter(timestamp) + getSign(String(timestamp), 1).slice(0, 9);
    ...
    window.fetch = async function (...args) {
      ...
      if (request.url.indexOf("area_accessible") > -1) {
        ...
        if (request.url.indexOf("activation-code") > -1) {
          ...
          if (request.url.indexOf("revoke") > -1) {
            ...
            if (request.url.indexOf("signin") > -1 || request.url.indexOf("verify") > -1 || request.url.indexOf("verification") > -1 || request.url.indexOf("token") > -1) {
              ...
              if (request.url.indexOf("signup") > -1) {
                ...
                if (request.url.indexOf("init") > -1) {
                  ...
                  if (requestBody.meta) {
                    ...
                    let captchaString = requestBody.client_id + requestBody.meta.client_version + requestBody.meta.package_name + RandomDeviceId + requestBody.meta.timestamp;
                    if (setCaptchaSign) {
                      requestBody.meta.captcha_sign = '1.' + getSign(captchaString, indexc);
                    }
                  }
                }
              }
      ...
      const newRequest = new Request(request, {
        ...
        'headers': newHeaders,
        'body': contentType.includes("application/json") ? JSON.stringify(requestBody) : request.body
      });
